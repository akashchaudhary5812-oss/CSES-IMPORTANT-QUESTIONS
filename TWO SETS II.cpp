...
bool revsort(pair<int,int>& a, pair<int,int>& b) {if(a.ss != b.ss) return b.ss > a.ss; return b.ff > a.ff;}
...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int dp[501][501 * 501];
int helper(int i, int s, int sum, vector<int>& arr){
    if(i >= sz(arr)){
       return s == (sum / 2); 
    }
    if(dp[i][s] != -1) return dp[i][s];
    ll way1 = helper(i + 1,s + arr[i],sum,arr);
    ll way2 = helper(i + 1,s,sum,arr);
    dp[i][s] = mod_add(way1,way2,MOD);
    return dp[i][s];
}
void solve() {
     int tc;
     ...
     while(tc--){
          ...
          cout << mod_div(helper(0,0,sum,arr),2,MOD) << nline;
     } 
}
int main() {
 #ifndef ONLINE_JUDGE
